...
public class ReverseClient {
    public static void main(String[] args) throws IOException {
        ...
        String filePath = "C:\\Users\\admin\\Desktop\\tcp\\src\\test.txt"; // 文件路径
        ...
        int bytesRead = 0;
        String path = "C:\\Users\\admin\\Desktop\\tcp\\src\\reverse.txt";
        File f = new File(path);
        FileWriter fw = new FileWriter(f);
        for (int i = 0; i < N; i++) {
            int length;
            if(i == N-1){
                length = fileData.length - bytesRead;
            }else{
                length = Lmin + new Random().nextInt(Lmax - Lmin);
            }
            int type3 = 3;
            dos.writeInt(type3);
